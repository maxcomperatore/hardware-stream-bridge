#!/usr/bin/env python3
"""Send 24-hour abandoned checkout recovery emails via Resend from GitHub Actions."""
...
def http_json(method: str, url: str, body: dict | None = None) -> dict:
    headers = {"Authorization": f"Bearer {CRON_SECRET}", "User-Agent": "bipluk-abandoned-checkout/1.0"}
    data = None
    if body is not None:
...
def send_resend(to: str, subject: str, body: str, html: str, from_addr: str, reply_to: str) -> tuple[bool, str | None]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bipluk-abandoned-checkout/1.0",
...
def main() -> int:
    ...
    from_addr = queue.get("from", "Max from bipluk <support@bipluk.com>")
    reply_to = queue.get("reply_to", SMTP_REPLY_TO)
    ...
    for user in users:
        ...
        ok, err = send_resend(email, subject, text_body, html, from_addr, reply_to)
        if ok:
            sent.append({"id": user["id"], "email": email})
            print(f"sent abandoned checkout email: {masked}")
        else:
            failed.append({"id": user["id"], "email": email, "error": err})
            print(f"failed abandoned checkout email: {masked} — {err}", file=sys.stderr)
        time.sleep(0.6)

    result = http_json(
        "POST",
        f"{SITE}/api/cron/abandoned-checkout-ack",
        {
            "sent": sent,
            "failed": failed,
    ...
    print(f"Abandoned checkout ack completed: {len(sent)} sent, {len(failed)} failed")
