...
def main() -> int:
    ...
    from_addr = queue.get("from", "bipluk gang <support@bipluk.com>")
    reply_to = queue.get("reply_to", "halfradiationllc@gmail.com")
    ...
    for user in users:
        ...
        if ok:
            ...
            print(f"sent abandoned checkout email: {user['email']}")
        else:
            failed.append({"id": user["id"], "email": user["email"], "error": err})
            print(f"failed abandoned checkout email: {user['email']} — {err}", file=sys.stderr)
        time.sleep(0.6)
