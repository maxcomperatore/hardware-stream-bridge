...
def _allow_marketing_asset(request: Request, filename: str, token):
    if filename not in PROTECTED_MARKETING_ASSETS:
        return False
    # 1. Valid signed token allows access immediately
    if token and _marketing_serializer:
        try:
            if _marketing_serializer.loads(token, max_age=60 * 60 * 24 * 30) == filename:
                return True
        except Exception:
            pass
    # 2. Preview bots (Discord, Twitter, WhatsApp, Googlebot)
    if _is_preview_bot(request.headers.get("user-agent", "")):
        return True
    # 3. Same-origin / same-site request
    ...
    if _is_our_referer(request.headers.get("referer", "")):
        return True
    # 5. Mobile browsers, Safari, or in-app webviews may omit referer and sec-fetch-site
    host = (request.headers.get("host") or "").lower().split(":", 1)[0]
    if host in OUR_HOSTS or host.endswith(".vercel.app") or not host:
        return True
    return False
...
def get_user_avatar_url(email: str) -> str:
    idx = calculate_avatar_idx(email)
    return f"{SITE_BASE}/static/avatars/Simple%20colors/Icon{idx}.png"
...
templates.env.globals["get_user_avatar_url"] = get_user_avatar_url
...
async def serve_marketing_asset(filename: str, request: Request, t=None):
    ...
    if not os.path.isfile(asset_path):
        fallback_path = os.path.join(BASE_DIR, "static", filename)
...
async def add_security_headers(request: Request, call_next):
    ...
    response.headers["Permissions-Policy"] = "camera=(), microphone=(), geolocation=(), midi=(self)"
...
# Intercept root favicon, touch icons, and web manifest for direct crawlers
...
async def apple_touch_icon():
    path = os.path.join(BASE_DIR, "static", "apple-touch-icon.png")
    if os.path.exists(path):
...
async def site_webmanifest():
    path = os.path.join(BASE_DIR, "static", "site.webmanifest")
    if os.path.exists(path):
...
def assert_cron_authorized(request: Request) -> None:
    """GitHub Actions cron — bearer CRON_SECRET only. Fails closed unconditionally."""
...
def password_needs_rehash(hashed_password: str) -> bool:
    """Returns True if the password was stored using legacy unsalted SHA-256 and needs upgrade to bcrypt."""
    return bool(hashed_password) and not (hashed_password.startswith("$2b$") or hashed_password.startswith("$2a$"))


# Layer-7 In-Memory Sliding-Window Rate Limiter
...
def enforce_rate_limit(request: Request, action: str, max_requests: int = 10, window_seconds: int = 60):
    ...
    allowed, retry_after = auth_rate_limiter.is_allowed(key, max_requests, window_seconds)
    if not allowed:
        trigger_alert(
            "rate_limit_exceeded",
            f"Rate limit exceeded for IP `{client_ip}` on action `{action}`.",
            {"ip": client_ip, "action": action, "retry_after": retry_after},
...
async def do_login(request: Request, email: str = Form(...), password: str = Form(...), next: str = Form(None)):
    ...
    # Transparently upgrade legacy unsalted SHA-256 to salted bcrypt
    if password_needs_rehash(user.get("hashed_password", "")):
        try:
            upgraded_hash = hash_password(password)
            database.update_user_password(email, upgraded_hash)
            logger.info(f"Transparently upgraded legacy password hash to bcrypt for: {email}")
        except Exception as e:
            logger.warning(f"Could not auto-upgrade legacy password for {email}: {e}")
...
def verify_browser_email_token(token: str, email: str, expected_origin: str) -> bool:
    """
    Verifies WICG Email Verification Token (SD-JWT+KB) generated by Chrome/Mailbox providers.
    Validates token payload structure, audience, and email claim match.
    """
    try:
        import base64, json
        ...
        disclosed_email = None
        for disc in parts[1:-1]:
            try:
                disc_pad = disc + "=" * ((4 - len(disc) % 4) % 4)
                disc_arr = json.loads(base64.urlsafe_b64decode(disc_pad.encode("ascii")).decode("utf-8"))
                if isinstance(disc_arr, list) and len(disc_arr) >= 3 and disc_arr[1] == "email":
                    disclosed_email = str(disc_arr[2]).lower().strip()
            except Exception:
                continue

        token_email = (payload.get("email") or disclosed_email or "").lower().strip()
        if token_email and token_email == email.lower().strip():
            return True
        if payload.get("iss") and ("google" in payload.get("iss", "").lower() or "accounts" in payload.get("iss", "").lower()):
            if token_email == email.lower().strip():
                return True
    except Exception as e:
        logger.warning(f"EVT validation exception: {e}")
    return False

@app.post("/auth/magic-request")
async def do_magic_request(request: Request):
    enforce_rate_limit(request, "magic_request", max_requests=5, window_seconds=60)
    import secrets, time
    ...
    try:
        ...
        if "application/json" in content_type:
            ...
            evt_token = data.get("token")
        else:
            form = await request.form()
            email = form.get("email", "")
            next_url = form.get("next")
            evt_token = form.get("token")
    except Exception:
        email = ""
        next_url = None
        evt_token = None
    ...
    # ⚡ WICG Email Verification API (Chrome Origin Trial) Instant Silent Auth
    if evt_token and verify_browser_email_token(evt_token, email_clean, SITE_BASE):
        user = database.get_user_by_email(email_clean)
        if not user:
            user = database.create_user(email_clean, tier="free")
            logger.info(f"New user auto-registered via Chrome Email Verification API: {email_clean}")
        
        if user.get("tier") == "banned":
            return JSONResponse({"ok": False, "error": "Your account has been suspended."}, status_code=403)
            
        database.record_user_login(user["id"])
        redirect_target = safe_next_url(next_url) if next_url else "/home"
        resp = JSONResponse({"ok": True, "verified_instant": True, "redirect": redirect_target})
        resp.set_cookie(
            key="session_user",
        ...
        logger.info(f"User instantly logged in via Chrome Email Verification API: {email_clean}")
        trigger_alert(
            "evt_login_success",
            f"User `{email_clean}` verified instantly via Chrome Email Verification API.",
            {"email": email_clean},
            distinct_id=email_clean
        )
        return resp
    ...
    <div style="background-color: #000000; padding: 40px 20px; font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'SF Pro Display', 'Helvetica Neue', Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
        <div style="max-width: 460px; margin: 0 auto; background-color: #121215; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 20px; padding: 36px 32px; text-align: center; box-shadow: 0 20px 48px rgba(0, 0, 0, 0.7);">
            ...
            <h2 style="color: #ffffff; font-size: 17px; font-weight: 600; letter-spacing: -0.01em; margin: 0 0 8px 0;">Sign in to your vault</h2>
            <p style="color: #a1a1a6; font-size: 14px; line-height: 22px; margin: 0 0 24px 0;">
                Click below to securely sign in to <span style="color: #ffffff; font-family: -apple-system-ui-monospace, 'SF Mono', monospace;">{email_clean}</span>:
            </p>
            ...
            <p style="color: #52525b; font-size: 11px; margin: 0;">
                If you didn't request this sign-in link, you can safely ignore this message.
            </p>
        </div>
    </div>
    """
...
async def do_magic_code_verify(request: Request, background_tasks: BackgroundTasks, email: str = Form(...), code: str = Form(...), next: str = Form(None)):
    enforce_rate_limit(request, "magic_code_verify", max_requests=10, window_seconds=60)
    import secrets, hashlib, time
    email_clean = email.lower().strip()
...
async def do_forgot_password(request: Request, email: str = Form(...)):
    enforce_rate_limit(request, "forgot_password", max_requests=5, window_seconds=60)
    import secrets, time
    email_clean = email.lower().strip()
    user = database.get_user_by_email(email_clean)
    ...
    <div style="background-color: #000000; padding: 40px 20px; font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'SF Pro Display', 'Helvetica Neue', Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
        <div style="max-width: 460px; margin: 0 auto; background-color: #121215; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 20px; padding: 36px 32px; text-align: center; box-shadow: 0 20px 48px rgba(0, 0, 0, 0.7);">
            ...
            <h2 style="color: #ffffff; font-size: 17px; font-weight: 600; letter-spacing: -0.01em; margin: 0 0 8px 0;">Password reset code</h2>
            <p style="color: #a1a1a6; font-size: 14px; line-height: 22px; margin: 0 0 24px 0;">
                Enter this confirmation code to set a new password for <span style="color: #ffffff; font-family: -apple-system-ui-monospace, 'SF Mono', monospace;">{email_clean}</span>:
            </p>

            <!-- Apple-Style Code Box -->
            <div style="background-color: #18181c; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 14px; padding: 22px; margin-bottom: 24px;">
                <span style="font-family: -apple-system-ui-monospace, 'SF Mono', 'Geist Mono', Menlo, monospace; font-size: 32px; font-weight: 600; letter-spacing: 8px; color: #ffffff; display: block; text-indent: 8px;">
                    {code}
                </span>
                <span style="color: #86868b; font-size: 11px; margin-top: 8px; display: block;">Expires in 30 minutes</span>
            </div>
...
async def verify_reset_code(request: Request, email: str = Form(...), code: str = Form(...)):
    enforce_rate_limit(request, "verify_reset_code", max_requests=10, window_seconds=60)
    import secrets, time
    email_clean = email.lower().strip()
    code_clean = code.strip()
...
async def do_reset_password_confirm(request: Request, token: str = Form(...), password: str = Form(...), confirm_password: str = Form(...)):
    enforce_rate_limit(request, "reset_password_confirm", max_requests=5, window_seconds=60)
    import time
    token_data = RESET_TOKENS.get(token)
    if not token_data or time.time() > token_data.get("expires", 0):
...
async def test_welcome_email(email: str = "max@gmail.com", send: str = None):
    ...
    avatar_url = get_user_avatar_url(email)
    ...
    if send == "1":
        ...
        plain_body = (
            f"Hi {first_name_cap},\n\n"
            "We built Bipluk because we were sick of two things: 1990s abandonware drivers failing during soundcheck, "
            "and watching decades of custom patches vanish into the void when a 3-volt internal coin battery quietly dies.\n\n"
            "No legacy drivers or desktop software sprawl. Just byte-for-byte SysEx precision running directly inside your browser.\n\n"
...
def send_battery_warning_email(email: str) -> tuple[bool, str]:
    email_clean = email.lower().strip()
    unsubscribe_token = cookie_signer.sign(email_clean.encode()).decode()
    avatar_url = get_user_avatar_url(email_clean)
...
def send_vip_purchase_email(email: str, plan: str = "personal") -> tuple[bool, str]:
    ...
    name_part = email_clean.split('@')[0]
    first_name = re.split(r'[\._-]', name_part)[0]
    first_name_cap = first_name.capitalize() if first_name else "Producer"
...
async def test_smart_email(email: str = "max@gmail.com", type: str = "battery", send: str = None):
    ...
    if type == "battery":
        if send == "1":
            ok, err = send_battery_warning_email(email_clean)
            return {"status": "sent" if ok else "failed", "error": err, "type": "battery", "email": email_clean}
        template = templates.get_template("email_battery_warning.html")
        ...
        if send == "1":
            ok, err = send_vault_limit_email(email_clean)
            return {"status": "sent" if ok else "failed", "error": err, "type": "vault_limit", "email": email_clean}
        template = templates.get_template("email_vault_limit.html")
        return HTMLResponse(content=template.render({"email": email_clean, "unsubscribe_token": "test"}))
    elif type == "vip_purchase":
        if send == "1":
            ok, err = send_vip_purchase_email(email_clean)
            return {"status": "sent" if ok else "failed", "error": err, "type": "vip_purchase", "email": email_clean}
        template = templates.get_template("email_purchase_vip.html")
        return HTMLResponse(content=template.render({
            "email": email_clean,
            "first_name": first_name_cap,
...
async def do_signup(
    ...
    enforce_rate_limit(request, "signup", max_requests=5, window_seconds=60)
    email_clean = email.lower().strip()
...
async def export_all_banks(request: Request):
    ...
    for bank in banks:
        try:
            ...
            synth_model_lower = str(bank.get("synth_model", "")).lower()
            if "prophet" in synth_model_lower or "sequential" in synth_model_lower:
                import parser
                import re
                ...
                if full_bank:
                    ...
                    for idx, p in enumerate(patches):
                        ...
                        raw_name = p.get("name", f"Patch {idx+1:02d}") if isinstance(p, dict) else str(p)
                        clean_name = re.sub(r'^\d{2}\s*[\:\-\.]?\s*', '', raw_name).strip()
                        patch_names.append(f"{slot_label} {clean_name}")
...
async def create_bank(
    ...
    if user["tier"] != "premium" and len(banks) >= 1:
        ...
        if "hx-request" in request.headers:
            return HTMLResponse(content="", headers={"HX-Trigger": "triggerUpgradeModal"})
        dest = request.headers.get("sec-fetch-dest", "")
        if dest in ("empty", "cors") or request.headers.get("x-requested-with") == "XMLHttpRequest":
            return JSONResponse(status_code=402, content={"error": "vault_limit_reached", "message": "That first vault was on us. Upgrade to unlock unlimited vaults."})
        return RedirectResponse(url="/home?upgrade=1", status_code=303)
...
async def mock_checkout_success(email: str, plan: str = "personal"):
    ...
    if settings.IS_PRODUCTION or os.environ.get("VERCEL") or (STRIPE_SECRET_KEY and not STRIPE_SECRET_KEY.startswith("sk_test_")):
...
async def llms_txt():
    ...
    lines = [
        ...
        "- **HN product signal:** pricing backlash led to public $99 Personal lifetime framing; .syx export guarantee",
...
async def unsubscribe_page(request: Request, token: str = "", email: str = ""):
    ...
    if token:
        try:
            target_email = cookie_signer.unsign(token).decode()
        except BadSignature:
    ...
    if target_email and not error:
        ...
        trigger_alert(
            ...
            f"User `{target_email}` has unsubscribed from the newsletter.",
            {"email": target_email},
...
async def test_onboarding_email(email: str = "test@example.com", send: str = "1"):
    if send == "1":
...
async def test_inactivity_warning(email: str = "test@example.com", send: str = "1"):
    if send == "1":
...
async def test_abandoned_checkout(email: str = "test@example.com", send: str = "1"):
