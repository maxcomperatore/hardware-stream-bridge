...
def main() -> int:
    ...
    for user in users:
        ...
        if ok:
            ...
            print(f"sent re-engagement: {masked}")
        else:
            failed.append({"id": user["id"], "email": email, "error": err})
            print(f"failed re-engagement: {masked} — {err}", file=sys.stderr)
        time.sleep(0.6)
    ...
    print(f"Reengagement ack completed: {len(sent)} sent, {len(failed)} failed")
